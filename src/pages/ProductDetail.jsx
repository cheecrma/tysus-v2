...
const productsData = {
  ...
  "structure-pipe": {
    ...
    detailImages: [structurePipeImg, circularPipeImg, rectangularPipeImg, seamlessPipeImg],
    features: [
      "높은 강도와 내구성",
      "원형 및 각형 규격 다양",
      "건축 구조물 적합",
  ...
  if (!product) {
    return (
      <div className="page-container" style={{ textAlign: "center", paddingTop: "150px" }}>
        <h2>제품을 찾을 수 없습니다.</h2>
        <Link to="/products" className="btn-navy" style={{ marginTop: "2rem", display: "inline-flex" }}>
          <ArrowLeft size={18} />
          제품 목록으로
        </Link>
      </div>
  ...
  return (
    <div className="page-container">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3rem", marginBottom: "3rem" }}>
        ...
        >
          <span className="product-tag" style={{ marginBottom: "1rem", display: "inline-block" }}>
            {product.category}
          </span>
          <h1 style={{ fontSize: "2rem", fontWeight: 800, color: "#1a2a4a", marginBottom: "1rem" }}>
            {product.name}
          </h1>
          ...
          <div style={{ marginBottom: "2rem" }}>
            <h3 style={{ fontSize: "1rem", fontWeight: 700, color: "#1a2a4a", marginBottom: "1rem" }}>
              제품 특징
            </h3>
            <ul style={{ listStyle: "none", padding: 0 }}>
              {product.features.map((feature, idx) => (
                ...
                >
                  <ChevronRight size={16} color="#2c4a7c" />
                  {feature}
                </li>
              ))}
            </ul>
          ...
          {/* 문의 버튼 */}
          <Link to="/contact" className="btn-navy" style={{ display: "inline-flex" }}>
            <Phone size={18} />
            제품 문의하기
          </Link>
        </motion.div>
      ...
      >
        <h3 style={{ fontSize: "1.25rem", fontWeight: 700, color: "#1a2a4a", marginBottom: "1.5rem" }}>
          제품 규격
        </h3>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1.5rem" }}>
          {product.specs.map((spec, idx) => (
            <div key={idx}>
              <p style={{ fontSize: "0.85rem", color: "#64748b", marginBottom: "0.25rem" }}>{spec.label}</p>
              <p style={{ fontSize: "1rem", fontWeight: 600, color: "#1a2a4a" }}>{spec.value}</p>
            </div>
          ))}
        </div>
      </motion.div>
        ...
        >
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "1rem" }}>
            {product.detailImages.map((img, idx) => (
              ...
              >
                <img src={img} alt="" style={{ width: "100%", height: "250px", objectFit: "cover" }} />
              </div>
            ))}
          </div>
        </motion.div>
      ...
      >
        <h3>다른 제품도 확인해 보세요</h3>
        <p>태영스텐레스의 다양한 스테인리스 제품을 만나보세요.</p>
